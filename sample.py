...
def dropInitialPoints(post):
  if readExcel and not checkMode:
...
def main():
  ...
  fig.savefig(directory + f'/Mod vs Avg CNT diam at {truncationValue}.png', bbox_inches='tight')
  
  fig, ax = plt.subplots(figsize=(10,6))
  ax.set_yscale('log')
  sns.scatterplot('CNT_Diameter', 'Modulus', data=df, hue='Sample')
  plt.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0)
  ...
  plt.show()
  pdf.savefig(fig, bbox_inches='tight')
  fig.savefig(directory + f'/Mod vs CNT diam at {truncationValue}.png', bbox_inches='tight')
  ...
  for key in xySampleData:
    ...
    fig, ax = plt.subplots(figsize=(10,6))
    sns.scatterplot('Post_deflection', 'Wire_deflection', data=snsDF, hue='Location', alpha = 0.65)
    plt.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0)
    ...
    plt.show()
    pdf.savefig(fig, bbox_inches='tight')
    fig.savefig(directory + f'/Sample {key} at {truncationValue}.png', bbox_inches='tight')
